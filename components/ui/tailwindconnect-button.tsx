...
const TailwindconnectButton: React.FC<TailwindconnectButtonProps> = ({
    ...
    return (
      <Link
        ...
        className={cn(
          "bg-slate-800 no-underline group cursor-pointer relative" +
            " shadow-2xl shadow-zinc-900 rounded-md px" +
      ...
      >
        ...
        <span className="absolute inset-0 overflow-hidden rounded-md">
          <span className="absolute inset-0 rounded-md bg-[radial-gradient(75%_100%_at_50%_0%,rgba(56,189,248,0.6)_0%,rgba(56,189,248,0)_75%)] opacity-0 transition-opacity duration-500 group-hover:opacity-100"></span>
        </span>
  ...
  return (
    <button
      onClick={onClick}
      className={cn(
        "bg-slate-800 no-underline group cursor-pointer relative shadow-2xl" +
          " shadow-zinc-900 roundedmdlg p-px text-xs font-semibold leading-6" +
          " text-white inline-block",
        className
      )}
    >
